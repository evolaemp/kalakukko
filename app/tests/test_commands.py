...
class HarvestLanguageLocationsTestCase(TestCase):
	fixtures = ['languages.json']
	...
	def test_nargs(self):
		for args in (
			...
			with self.assertRaises(CommandError):
				call_command('harvest_language_locations', *args, **self.opts)
	
	def test_command(self):
		call_command('harvest_language_locations', *self.args, **self.opts)
		self.assertEqual('', self.stderr.getvalue())
		...
		rus = Language.objects.get(iso_639_3='rus')
